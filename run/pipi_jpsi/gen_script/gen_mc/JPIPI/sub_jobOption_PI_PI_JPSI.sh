...
for ECM in ${ECMS[@]}; do
    ...
    sh jobOptions_sim_sig_PI_PI_JPSI_JPIPI_${ECM}_705.sh 0 99 $ECM ${CMS[$COUNT]} 5000 ${RUNNO_LOW[$COUNT]} ${RUNNO_UP[$COUNT]}
    sh jobOptions_rec_sig_PI_PI_JPSI_JPIPI_${ECM}_705.sh 0 99 $ECM
    ...
    sh subSimRec.sh jobOptions_sim_sig_PI_PI_JPSI_JPIPI_$ECM jobOptions_rec_sig_PI_PI_JPSI_JPIPI_$ECM subSimRec_PI_PI_JPSI_$ECM 0 99
    COUNT=$((${COUNT}+1))
done
