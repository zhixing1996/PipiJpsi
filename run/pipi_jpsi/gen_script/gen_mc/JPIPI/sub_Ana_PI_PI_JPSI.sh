...
for ECM in ${ECMS[@]}; do
    ...
    cd $WORKAREA/run/pipi_jpsi/jobs_text/mc/DDPI/$ECM
    rm -rf mc_PI_PI_JPSI_JPIPI_$ECM*txt
    cp -rf $WORKAREA/python/make_mc.py ./
    cp -rf $WORKAREA/python/tools.py ./
    ./make_mc.py /besfs/groups/cal/dedx/$USER/bes/PipiJpsi/run/pipi_jpsi/dst/mc/JPIPI/$ECM mc PI_PI_JPSI JPIPI pipi_jpsi $ECM 2
